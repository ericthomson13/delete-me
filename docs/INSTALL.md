...
| `DELETE_ME_API_KEY` | FastAPI service (docker self-host) | no auth enforced; every endpoint is open. Set this when running with remote clients. `/health` is always exempt for liveness probes. |
